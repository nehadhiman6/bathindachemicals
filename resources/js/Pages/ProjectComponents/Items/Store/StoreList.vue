...
import StoreForm from '@/Pages/ProjectComponents/Items/Store/StoreForm.vue';
...
const setTable = () => {
    state.table = $('#stores_list').DataTable({
        "processing": true,
        "serverSide": true,
        "searchDelay": 700,
        "ordering": true,
        ...
        pageLength: 10,
        "ajax": {
            "url": base_url.value+"/stores-list",
            "type": "GET",
        },
        ...
        columns: [{
            ...
            },
            {
                title: 'Actions',
                orderable: false,
                visible:canAny(page.props.granted_permissions,['stores-modify']),
                data: 'id',
                render: function (data, type, row, meta) {
        ...
        "drawCallback": function (settings) {
            $(".edit-item").unbind('click').on('click', function (e) {
                state.formOpen = false;
                editStore(e.target.dataset.itemId);
            });
...
<template>
    <AppLayout title="Stores">
        ...
        <div>
            <store-form v-if="state.formOpen" @resetForm="resetForm" :form_id="state.form_id">
            </store-form>
            <ListWrapper title="Stores List">
                <template #button>
                    <ButtonComp @buttonClicked="state.formOpen = true" type="new" v-if="state.formOpen == false && canAny(page.props.granted_permissions,['stores-add'])">
                        <i class="fa-solid fa-plus mr-2"></i> New Store
                    </ButtonComp>
                </template>
                <template #table>
                    <table id="stores_list" class="row-border stripe" width="100%">
                    </table>
                </template>
            </ListWrapper>
        </div>
