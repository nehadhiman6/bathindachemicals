...
<script setup>
    ...
    const data = reactive({ create_url:'stores' });
    const pageTitle = computed(() => props.form_id > 0 ? 'Update  Store':'Add Store');
    ...
    const getStore = () =>{
        axios.get(base_url.value+'/stores/'+props.form_id+'/edit')
        .then(function(response){
