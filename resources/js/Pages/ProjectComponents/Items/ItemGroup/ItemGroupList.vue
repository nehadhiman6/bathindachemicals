...
import ItemGroupForm from '@/Pages/ProjectComponents/Items/ItemGroup/ItemGroupForm.vue';
import globalMixin from '../../../../globalMixin';

const { base_url,snakeCaseToString,canAny} = globalMixin();
const page = usePage();
const props = defineProps(['master_type']);
const group_type = computed(() => props.master_type == 'main_groups' ? 'M':( props.master_type == 'sub_groups' ? 'S1' :'S2'));
...
const setTable = () => {
    state.table = $('#type_master_list').DataTable({
        ...
        pageLength: 10,
        "ajax": {
            "url": base_url.value+"/item-groups-list",
            "type": "GET",
            "data":function(d){
        ...
        columns: [{
            ...
            {
                title: 'Actions',
                visible:canAny(page.props.granted_permissions,['party-sub-groups-modify','clients-modify','vendors-modify']),
                orderable: false,
                data: 'id',
                render: function (data, type, row, meta) {
        ...
        "drawCallback": function (settings) {
            $(".edit-item").unbind('click').on('click', function (e) {
                state.formOpen = false;
                editItemGroup(e.target.dataset.itemId);
            });
...
<template>
    <AppLayout :title="title">
        ...
        <div>
            <item-group-form v-if="state.formOpen" @resetForm="resetForm" :form_id="state.form_id" :title="title" :create_url="master_type" :group_type="group_type">
            </item-group-form>
            <ListWrapper :title="title +' List'">
                <template #button>
                    <ButtonComp @buttonClicked="state.formOpen = true" type="new" v-if="state.formOpen == false && canAny(page.props.granted_permissions,['party-sub-groups-modify','clients-modify','vendors-modify'])">
