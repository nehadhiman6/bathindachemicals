...
import DistrictForm from '@/Pages/ProjectComponents/Masters/District/DistrictForm.vue';
...
const setTable = () => {
    state.table = $('#districts_list').DataTable({
        "processing": true,
        "serverSide": true,
        "searchDelay": 700,
        "ordering": true,
        ...
        pageLength: 10,
        "ajax": {
            "url": base_url.value+"/districts-list",
            "type": "GET",
        },
        ...
        columns: [{
            ...
            },
            {
                title: 'Actions',
                orderable: false,
                visible:canAny(page.props.granted_permissions,['districts-modify']),
                data: 'id',
                render: function (data, type, row, meta) {
...
<template>
    <AppLayout title="districts">
        ...
        <div>
            <district-form v-if="state.formOpen" @resetForm="resetForm" :form_id="state.form_id" :states="props.states">
            </district-form>
            <ListWrapper title="districts List">
                <template #button>
                    <ButtonComp @buttonClicked="state.formOpen = true" type="new" v-if="state.formOpen == false && canAny(page.props.granted_permissions,['districts-add'])">
                        <i class="fa-solid fa-plus mr-2"></i> New District
                    </ButtonComp>
                </template>
                <template #table>
                    <table id="districts_list" class="row-border stripe" width="100%">
                    </table>
                </template>
            </ListWrapper>
        </div>
