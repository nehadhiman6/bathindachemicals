...
import FixRateForm from '@/Pages/ProjectComponents/Masters/FixRate/FixRateForm.vue';
...
const setTable = () => {
    state.table = $('#fix_rate_list').DataTable({
        "processing": true,
        "serverSide": true,
        "searchDelay": 700,
        "ordering": true,
        ...
        pageLength: 10,
        "ajax": {
            "url": base_url.value+"/fix-rates-list",
            "type": "GET",
            "data":function(c){
            }
        },
        ...
        columns: [{
            ...
            },
            {
                title: 'Actions',
                orderable: false,
                // visible:canAny(page.props.granted_permissions,['fix-rates-modify']),
                data: 'id',
                render: function (data, type, row, meta) {
                    var str = '';
                        if(canAny(page.props.granted_permissions,['fix-rates-modify']))
                        ...
                        if(canAny(page.props.granted_permissions,['fix-rates-view']))
        ...
        "drawCallback": function (settings) {
            $(".edit-item").unbind('click').on('click', function (e) {
                ...
                editFixRate(e.target.dataset.itemId);
            });
...
<template>
    <AppLayout title=" Fix Rate">
        ...
        <div>
            <fix-rate-form v-if="state.formOpen" :readonly="state.readonly" @resetForm="resetForm" :form_id="state.form_id" >
            </fix-rate-form>
            <ListWrapper title=" Fix Rate">
                <template #button>
                    <ButtonComp @buttonClicked="state.formOpen = true" type="new" v-if="state.formOpen == false && canAny(page.props.granted_permissions,['fix-rates-add'])">
                        <i class="fa-solid fa-plus mr-2"></i> New Fix Rate
                    </ButtonComp>
                </template>
                <template #table>
                    <table id="fix_rate_list" width="100%" class="row-border stripe">
                    </table>
                </template>
            </ListWrapper>
        </div>
