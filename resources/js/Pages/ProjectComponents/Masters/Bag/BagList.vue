...
import BagForm from '@/Pages/ProjectComponents/Masters/Bag/BagForm.vue';
...
const setTable = () => {
    state.table = $('#bags_list').DataTable({
        "processing": true,
        "serverSide": true,
        "searchDelay": 700,
        "ordering": true,
        ...
        columns: [{
...

            {
                title: 'Actions',
                orderable: false,
                visible:canAny(page.props.granted_permissions,['bags-modify']),
                data: 'id',
                render: function (data, type, row, meta) {
...
<template>
    <AppLayout title="Bags">
        ...
        <div>
            <bag-form v-if="state.formOpen" @resetForm="resetForm" :form_id="state.form_id">
            </bag-form>
            <ListWrapper title="Bags List">
                <template #button>
                    <ButtonComp @buttonClicked="state.formOpen = true" type="new" v-if="state.formOpen == false && canAny(page.props.granted_permissions,['bags-add'])">
                        <i class="fa-solid fa-plus mr-2"></i> New Bag
                    </ButtonComp>
                </template>
                <template #table>
                    <table id="bags_list" class="row-border stripe" width="100%">
                    </table>
                </template>
            </ListWrapper>
        </div>
