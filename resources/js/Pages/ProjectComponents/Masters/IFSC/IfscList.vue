...
import IfscForm from '@/Pages/ProjectComponents/Masters/IFSC/IfscForm.vue';
    ...
    state.table = $('#ifscs_list').DataTable({
        ...
        columns: [{
            ...
            },
            {
                ...
                data: 'name',
                "render": function (data, type, row, meta) {
                    var str = row.bank ? row.bank.name :'';
                    return  str;
                }
            },
            {
                ...
                orderable: false,
                data: 'id',
                 visible:canAny(page.props.granted_permissions,['ifscs-modify']),
                render: function (data, type, row, meta) {
...
<template>
    <AppLayout title="ifscs">
        <template #header>
        </template>
        <div>
            <ifsc-form v-if="state.formOpen" @resetForm="resetForm" :form_id="state.form_id" :banks="props.banks">
            </ifsc-form>
            <ListWrapper title="Ifsc List">
                <template #button>
                    <ButtonComp @buttonClicked="state.formOpen = true" type="new" v-if="state.formOpen == false && canAny(page.props.granted_permissions,['ifscs-add'])">
                        <i class="fa-solid fa-plus mr-2"></i>New Ifsc
                    </ButtonComp>
                </template>
                <template #table>
