...
import CountryForm from '@/Pages/ProjectComponents/Masters/Country/CountryForm.vue';
...
const setTable = () => {
    state.table = $('#countries_list').DataTable({
        "processing": true,
        "serverSide": true,
        "searchDelay": 700,
        "ordering": true,
        ...
        pageLength: 10,
        "ajax": {
            "url": base_url.value+"/countries-list",
            "type": "GET",
        },
        ...
        columns: [{
            ...
            },
            {
                title: 'Actions',
                orderable: false,
                visible:canAny(page.props.granted_permissions,['countries-modify']),
                data: 'id',
                render: function (data, type, row, meta) {
...
<template>
    <AppLayout title="Countries">
        ...
        <div>
            <country-form v-if="state.formOpen" @resetForm="resetForm" :form_id="state.form_id">
            </country-form>
            <ListWrapper title="Countries List">
                <template #button>
                    <ButtonComp @buttonClicked="state.formOpen = true" type="new" v-if="state.formOpen == false && canAny(page.props.granted_permissions,['countries-add'])">
                        <i class="fa-solid fa-plus mr-2"></i> New Country
                    </ButtonComp>
                </template>
                <template #table>
                    <table id="countries_list" class="row-border stripe" width="100%">
                    </table>
                </template>
            </ListWrapper>
        </div>
