...
import RateDifferenceForm from '@/Pages/ProjectComponents/Masters/RateDifference/RateDifferenceForm.vue';
...
const setTable = () => {
    state.table = $('#sale_contract_list').DataTable({
        ...
        pageLength: 10,
        "ajax": {
            "url": base_url.value+"/rate-difference-list",
            "type": "GET",
            "data":function(c){
        ...
        columns: [{
            ...
            },
            {
                title: 'Actions',
                orderable: false,
                // visible:canAny(page.props.granted_permissions,['rate-differences-modify']),
                data: 'id',
                render: function (data, type, row, meta) {
                    var str = '';
                    if(canAny(page.props.granted_permissions,['rate-differences-modify']))
                    ...
                    if(canAny(page.props.granted_permissions,['rate-differences-view']))
        ...
        "drawCallback": function (settings) {
            ...
            $(".new-item").unbind('click').on('click', function (e) {
                state.formOpen = false;
                copyRateDifference(e.target.dataset.itemId);
            });
        }
    });
...
<template>
    <AppLayout title=" Ltr Tin Differences">
        <template #header>
        ...
        <div>
            <rate-difference-form v-if="state.formOpen" :copy="state.copy" @resetForm="resetForm" :form_id="state.form_id" :close_qty="state.close_qty"
                :default_packing="default_packing" :readonly="state.readonly"
            >
            </rate-difference-form>
            <ListWrapper title=" Ltr Tin Differences">
                <template #button>
                    <ButtonComp @buttonClicked="state.formOpen = true" type="new" v-if="state.formOpen == false && canAny(page.props.granted_permissions,['rate-differences-add'])">
                        <i class="fa-solid fa-plus mr-2"></i> New  Ltr Tin Difference
                    </ButtonComp>
                </template>
                <template #table>
