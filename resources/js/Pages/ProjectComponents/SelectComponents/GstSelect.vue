<script setup>
   import {   ref,  computed, onMounted, onUnmounted,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updateGST','update:modelValue']);
    ...
    const props = defineProps(
        ...
        url:{default:'gsts/filtered',type: String},
        ...
        placeholder:{default:'Select GST',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'gst_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full'
                            :'gst_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full';
    });
    ...
    onUnmounted(() => {
        // $('.gst_'+props.index).select2('destroy');
        // console.log( $('.gst_'+props.index));
    });
    const intialiseSelect = ()=>{
        console.log("base_url");
        console.log(base_url.value);
        var itemComponent = $('.gst_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "gst_drop_"+props.index,
            ajax: {
                method:'POST',
                url: function() {
        ...
        .on('change', function () {
            ...
            var item = $('.gst_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateGST',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data.value != {}){
               emit('updateGST',item,props.index,selected_data.value);
            }
        })
        ...
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.gst_drop_'+props.index+' .select2-search__field').focus();
            }
        });

        if(typeof(props.index) !="undefined"){
            $('.gst_'+props.index).val(props.selected).change();
        }
    }
