<script setup>
   import {   ref,  computed,  onMounted,  onUnmounted,onBeforeUnmount, reactive} from 'vue';
    ...
    const emit = defineEmits(['updateSection','update:modelValue']);
    ...
    const props = defineProps(
    {
        ...
        url:{default:'tds-section/filtered',type: String},
        ...
        placeholder:{default:'Select Section',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'tds_section_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm block w-full'
                            :'tds_section_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm block w-full';
    });
    ...
    onBeforeUnmount(() => {
        $('.tds_section_'+props.index).select2('destroy');
    ...
    const intialiseSelect = ()=>{
        ...
        var itemComponent = $('.tds_section_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "tds_section_drop_"+props.index,
            ajax: {
                method:'POST',
                url: function() {
        ...
        .on('change', function () {
            var item = $('.tds_section_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateSection',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data != {}){
               emit('updateSection',item,props.index,selected_data);
            }
        })
        ...
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.tds_section_drop_'+props.index+' .select2-search__field').focus();
            }
       });

        if(typeof(props.index) !="undefined"){
            $('.tds_section_'+props.index).val(props.selected).change();
        }
    }
