<script setup>
   import {   ref,  computed,  onMounted,  onUnmounted,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updateState','update:modelValue']);
    ...
    const props = defineProps(
    {
        ...
        url:{default:'states/filtered',type: String},
        ...
        placeholder:{default:'Select State',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'state_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm block w-full'
                            :'state_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm block w-full';
    });
    ...
    const intialiseSelect = ()=>{
        ...
        var itemComponent = $('.state_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "state_drop_"+props.index,
            ajax: {
                method:'POST',
                url: function() {
        ...
        .on('change', function () {
            var item = $('.state_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateState',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data != {}){
               emit('updateState',item,props.index,selected_data);
            }
        })
        ...
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.state_drop_'+props.index+' .select2-search__field').focus();
            }
       });

        if(typeof(props.index) !="undefined"){
            $('.state_'+props.index).val(props.selected).change();
        }
    }
