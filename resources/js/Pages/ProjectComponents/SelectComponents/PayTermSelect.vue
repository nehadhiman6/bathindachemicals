<script setup>
   import {   ref,  computed, onMounted, onUnmounted,  reactive} from 'vue';
    ...
    const props = defineProps(
        ...
        url:{default:'pay-terms/filtered',type: String},
        ...
        placeholder:{default:'Select Pay Term',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'pay_term_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full'
                            :'pay_term_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full';
    });
    ...
    onUnmounted(() => {
        // $('.pay_term_'+props.index).select2('destroy');
        // console.log( $('.pay_term_'+props.index));
    });
    const intialiseSelect = ()=>{
        console.log("base_url");
        console.log(base_url.value);
        var itemComponent = $('.pay_term_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "pay_term_drop_"+props.index,
            ajax: {
                method:'POST',
                url: function() {
        ...
        .on('change', function () {
            ...
            var item = $('.pay_term_'+ props.index).val();
        ...
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.pay_term_drop_'+props.index+' .select2-search__field').focus();
            }
        });

        if(typeof(props.index) !="undefined"){
            $('.pay_term_'+props.index).val(props.selected).change();
        }
    }
