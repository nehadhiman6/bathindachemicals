<script setup>
   import {   ref,  computed, onMounted, onUnmounted,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updateIfsc','update:modelValue']);
    ...
    const props = defineProps(
        ...
        url:{default:'ifscs/filtered',type: String},
        ...
        placeholder:{default:'Select Ifsc',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'ifsc_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opaifsc-50 rounded-md shadow-sm mt-1 block w-full'
                            :'ifsc_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opaifsc-50 rounded-md shadow-sm mt-1 block w-full';
    });
    ...
    onUnmounted(() => {
        // $('.ifsc_'+props.index).select2('destroy');
        // console.log( $('.ifsc_'+props.index));
    });
    const intialiseSelect = ()=>{
        console.log("base_url");
        console.log(base_url.value);
        var itemComponent = $('.ifsc_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "ifsc_drop_"+props.index,
        ...
        .on('change', function () {
            ...
            var item = $('.ifsc_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateIfsc',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data.value != {}){
               emit('updateIfsc',item,props.index,selected_data.value);
            }
        })
        ...
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.ifsc_drop_'+props.index+' .select2-search__field').focus();
            }
       });

        if(typeof(props.index) !="undefined"){
            $('.ifsc_'+props.index).val(props.selected).change();
        }
    }

    const setName = (details) =>{
        if (!details.id) { return details.text; }
        var $details = $('<div class="flex flex-wrap -mx-3"><div class="w-full md:w-1/2 px-3 md:mb-0">' + details.text + '</div><div class="w-full md:w-1/2 px-3 md:mb-0">' + '</div></div>');
        return $details;
    }
