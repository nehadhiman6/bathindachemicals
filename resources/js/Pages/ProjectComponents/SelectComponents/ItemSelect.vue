<script setup>
   import {   ref,  computed,  onMounted,  onUnmounted,  reactive} from 'vue';
    ...
    const props = defineProps(
    {
        ...
        url:{default:'items/filtered',type: String},
        ...
        placeholder:{default:'Select Item ',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'item_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm block w-full'
                :'item_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm block w-full';
    });
    ...
    const intialiseSelect = ()=>{
        var itemComponent = $('.item_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "item_drop_"+props.index,
            ajax: {
                method:'POST',
                url: function() {
        ...
        .on('change', function () {
            var item = $('.item_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateItem',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data != {}){
               emit('updateItem',item,props.index,selected_data);
            }
        })
        ...
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.item_drop_'+props.index+' .select2-search__field').focus();
            }
       });

        if(typeof(props.index) !="undefined"){
            $('.item_'+props.index).val(props.selected).change();
        }
    }
