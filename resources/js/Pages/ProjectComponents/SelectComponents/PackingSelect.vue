<script setup>
   import {   ref,  computed,  onMounted,  onUnmounted,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updatePacking','update:modelValue']);
    ...
    const props = defineProps(
    {
        ...
        url:{default:'packings/filtered',type: String},
        ...
        placeholder:{default:'Select Packing',type:String},
        type:{default:'all',type:String},
        s_p_type:{default:'all',type:String},
        add_data:{default:'',type:String},
        multiple:{default:false,type:Boolean},
        allowClear:{default:true,type:Boolean},
        projectId:{default:0,type:[Number,String]},
        searchType:{default:'',type:String},
        error:{default:false},
        strictInclude:{default:false},
        includeOnly:{default:[],type:[Array]},
        bill_no:{default:'',type: [String,Number]},
        session:{default:'',type: [String,Number]},
        item_id:{default:'',type: [String,Number]},
        }
    );
    var selected_data = ref(0);

    const classes = computed(() => {
        return props.error ? 'packing_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm block w-full'
                            :'packing_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm block w-full';
    });
    ...
    const intialiseSelect = ()=>{
        ...
        var itemComponent = $('.packing_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "packing_drop_"+props.index,
            ajax: {
                method:'POST',
                url: function() {
                ...
                data: function (params) {
                    var query = {
                        ...
                        s_p_type:props.s_p_type,
                        search_type:props.searchType,
                        include_only: props.includeOnly,
                        strict_include :props.strictInclude == true ? 'Y':'N'
                    }
        ...
        .on('change', function () {
            var item = $('.packing_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updatePacking',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data != {}){
               emit('updatePacking',item,props.index,selected_data);
            }
        })
        ...
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.packing_drop_'+props.index+' .select2-search__field').focus();
            }
       });

        if(typeof(props.index) !="undefined"){
            $('.packing_'+props.index).val(props.selected).change();
        }
    }
