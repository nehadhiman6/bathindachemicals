<script setup>
   import {   ref,  computed, onMounted, onUnmounted,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updateBankAccount','update:modelValue']);
    ...
    const props = defineProps(
        ...
        url:{default:'bank-accounts/filtered',type: String},
        ...
        placeholder:{default:'Select Bank Account',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'bank_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full'
                            :'bank_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full';
    });
    ...
    onUnmounted(() => {
        // $('.bank_'+props.index).select2('destroy');
        // console.log( $('.bank_'+props.index));
    });
    const intialiseSelect = ()=>{
        console.log("base_url");
        console.log(base_url.value);
        var itemComponent = $('.bank_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "bank_account_drop_"+props.index,
            ajax: {
                method:'POST',
                url: function() {
                ...
                processResults: function (data,params) {
                    ...
                    data.results.forEach(function(ele){
                        ele.text = ele.bank_account_number;
                        ele.id = ele.bank_account_number;
                    });
        ...
        .on('change', function () {
            ...
            var item = $('.bank_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateBankAccount',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data.value != {}){
               emit('updateBankAccount',item,props.index,selected_data.value);
            }
        })
        ...
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.bank_account_drop_'+props.index+' .select2-search__field').focus();
            }
        });

        if(typeof(props.index) !="undefined"){
            $('.bank_'+props.index).val(props.selected).change();
        }
    }
