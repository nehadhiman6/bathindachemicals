<script setup>
   import {   ref,  computed, onMounted, onUnmounted,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updateBranch','update:modelValue']);
    ...
    const props = defineProps(
        ...
        url:{default:'login-branches/filtered',type: String},
        ...
        placeholder:{default:'Select Branch',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'branch_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full'
                            :'branch_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full';
    });
    ...
    onUnmounted(() => {
        // $('.branch_'+props.index).select2('destroy');
        // console.log( $('.branch_'+props.index));
    });
    const intialiseSelect = ()=>{
        console.log("base_url");
        console.log(base_url.value);
        var itemComponent = $('.branch_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "branch_drop_"+props.index,
            ajax: {
                method:'POST',
                url: function() {
        ...
        .on('change', function () {
            ...
            var item = $('.branch_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateBranch',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data.value != {}){
               emit('updateBranch',item,props.index,selected_data.value);
            }
        })
        .on('select2:clear',function(e){
            ...
            if(props.multiple == false){
                ...
                emit('updateBranch',val);
            }
        })
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.branch_drop_'+props.index+' .select2-search__field').focus();
            }
        });

        if(typeof(props.index) !="undefined"){
            $('.branch_'+props.index).val(props.selected).change();
        }
    }
