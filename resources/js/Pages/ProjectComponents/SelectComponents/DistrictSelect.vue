<script setup>
   import {   ref,  computed, onMounted, onUnmounted,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updateDistrict','update:modelValue']);
    ...
    const props = defineProps(
        ...
        url:{default:'districts/filtered',type: String},
        ...
        placeholder:{default:'Select District',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'district_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opadistrict-50 rounded-md shadow-sm mt-1 block w-full'
                            :'district_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opadistrict-50 rounded-md shadow-sm mt-1 block w-full';
    });
    ...
    onUnmounted(() => {
        // $('.district_'+props.index).select2('destroy');
        // console.log( $('.district_'+props.index));
    });
    const intialiseSelect = ()=>{
        console.log("base_url");
        console.log(base_url.value);
        var itemComponent = $('.district_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "district_drop_"+props.index,
            ajax: {
                method:'POST',
                url: function() {
        ...
        .on('change', function () {
            ...
            var item = $('.district_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateDistrict',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data.value != {}){
               emit('updateDistrict',item,props.index,selected_data.value);
            }
        })
        ...
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.district_drop_'+props.index+' .select2-search__field').focus();
            }
        })
        ;

        if(typeof(props.index) !="undefined"){
            $('.district_'+props.index).val(props.selected).change();
        }
