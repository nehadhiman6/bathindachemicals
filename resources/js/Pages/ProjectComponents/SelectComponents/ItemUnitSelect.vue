<script setup>
   import {   ref,  computed,  onMounted,  onUnmounted,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updateItemUnit','update:modelValue']);
    ...
    const props = defineProps(
    {
        ...
        url:{default:'item-units/filtered',type: String},
        ...
        placeholder:{default:'Select Item Unit',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'item_unit_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm block w-full'
                            :'item_unit_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm block w-full';
    });
    ...
    const intialiseSelect = ()=>{
        ...
        var itemComponent = $('.item_unit_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "item_unit_drop_"+props.index,
            ajax: {
                method:'POST',
                url: function() {
        ...
        .on('change', function () {
            var item = $('.item_unit_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateItemUnit',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data != {}){
               emit('updateItemUnit',item,props.index,selected_data);
            }
        })
        ...
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.item_unit_drop_'+props.index+' .select2-search__field').focus();
            }
       });

        if(typeof(props.index) !="undefined"){
            $('.item_unit_'+props.index).val(props.selected).change();
        }
    }
