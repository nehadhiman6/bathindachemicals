<script setup>
   import {   ref,  computed, onMounted, onUnmounted,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updatePayTerm','update:modelValue']);
    ...
    const props = defineProps(
        ...
        url:{default:'type-masters/filtered',type: String},
        ...
        placeholder:{default:'Select Clients',type:String},
        type:{default:'all',type:String},
        multiple:{default:false,type:Boolean},
        allowClear:{default:true,type:Boolean},
        type:{default:'clients',type:[String]},
    ...
    const classes = computed(() => {
        return props.error ? 'type_master_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full'
                            :'type_master_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full';
    });
    ...
    onUnmounted(() => {
        // $('.type_master_'+props.index).select2('destroy');
        // console.log( $('.type_master_'+props.index));
    });
    const intialiseSelect = ()=>{
        console.log("base_url");
        console.log(base_url.value);
        var itemComponent = $('.type_master_'+props.index);
        var item = itemComponent.select2({
            ...
            data:props.initials,
            dropdownParent:$('body'),
            dropdownCssClass: "type_master_drop_"+props.index,
            ajax: {
                method:'POST',
                url: function() {
        ...
        .on('change', function () {
            ...
            var item = $('.type_master_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updatePayTerm',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data.value != {}){
               emit('updatePayTerm',item,props.index,selected_data.value);
            }
        })
        ...
        .on('select2:open', (e) => {
            if(props.multiple == false){
                document.querySelector('.type_master_drop_'+props.index+' .select2-search__field').focus();
            }
       });

        if(typeof(props.index) !="undefined"){
            $('.type_master_'+props.index).val(props.selected).change();
        }
    }
