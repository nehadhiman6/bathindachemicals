...
use App\Models\Attachment\YearlyResource;
use App\Models\Company\SaleContract;
use Illuminate\Database\Eloquent\Collection;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class YearlyResourceController extends Controller
{
    public function saveResources(Request $request, $resources_type){
        ...
        foreach($request->resources as $resource){
            if($resource['attachment_id'] > 0){
                $res = YearlyResource::firstOrNew(['resourceable_type'=>$target_class,'attachment_id'=>$resource['attachment_id'],
