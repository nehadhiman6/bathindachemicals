...
use App\Models\Attachment\CompanyAttachment;
...
class CompanyAttachmentController extends Controller
{
    public function store(Request $request)
    {
        ...
        if ($file) {
            ...
            if (config('bcl.upload_disk') == 's3') {
                $folderPath = config('bcl.aws_folder_soft1').'/' . companyAttachmentPath() . '/';
                $fileName =  $attachment->id . '.' . $attachment->file_ext;
                Storage::disk('s3')->putFileAs($folderPath, $file, $fileName);
            } else {
                $file->move(storage_path('app/' . companyAttachmentPath(). '/'), $attachment->id . '.' . $attachment->file_ext);
            }
            DB::commit();
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($attachment) {
            if (config('bcl.upload_disk') == 's3') {
                $file_path = '/' . config('bcl.aws_folder_soft1') . "/".companyAttachmentPath()."/" . $id . '.' . $attachment->file_ext;
            } else {
                $file_path = '/'.companyAttachmentPath().'/' . $attachment->id . '.' . $attachment->file_ext;
            }
    ...
    public function deleteAttachment(Request $request)
    {
        ...
        if ($file_id > 0) {
            DB::beginTransaction();
            $attachment =  CompanyAttachment::findOrFail($file_id);
            if ($attachment) {
                $image_path = storage_path('app/'.companyAttachmentPath().'/') . $attachment->id . '.' . $attachment->file_ext;
